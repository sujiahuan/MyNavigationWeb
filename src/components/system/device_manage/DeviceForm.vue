<template>
    ...
    >
        <a-form-model :model="form" ref='ruleForm' :rules="rules" :label-col="labelCol" :wrapper-col="wrapperCol">
            <a-row>
                <a-col :span="23">
                    <a-form-model-item :label="title=='新增'?'复制设备':'复制因子'" prop="copyDeviceId" :label-col="{span:4}" :wrapper-col="{span: 20}">
                ...
                <a-row>
                    <a-col :span="7">
                        <a-form-model-item label="模拟多设备" :label-col="{span:14}"
                                           :wrapper-col="{span: 8}">
                            <a-switch :checked="form.analogNumber==1?false:true" @change="onChange()"/>
                        </a-form-model-item>
                    </a-col>
                    <a-col :span="2">
                        <a-tooltip
                                title="模拟多个MN号，如当前MN号为ABC,当开启后输入个位，如2,那么MN号分别为：ABC1、ABC2。若输入十位，如12，那么MN号分别为：ABC01、ABC02、...、ABC12"
                                style="margin-top: 13px">
                            <a-icon type="question"/>
                        </a-tooltip>
                    </a-col>
                    <a-col :span="15">
                        <a-form-model-item label="设备数" prop="subpackageNumber" v-if="form.analogNumber!=1"
                                           :label-col="{span:6}" :wrapper-col="{span: 18}">
                            <a-input-number v-model="form.analogNumber" placeholder="请输入" :precision=0 :min=2
                                            style="width: 100%"/>
                        </a-form-model-item>
                    </a-col>
                </a-row>
